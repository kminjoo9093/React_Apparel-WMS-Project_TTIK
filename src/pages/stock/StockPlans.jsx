...
function StockPlans() {
  ...
  const [plansType, setPlansType] = useState(
    location.state?.activeTab || "InBound",
  );
  ...
  const fetchStockPlanList = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${SERVER_URL}/ttik/plans/${endpoint}/list`,
        {
          method: "GET",
          credentials: "include",
          headers: {
  ...
  return (
    <>
      <div>
        ...
        <p className={stylePlans.plansSubTitle}>
          입·출고 예정을 확인하고 관리하세요.
        </p>
          ...
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <div className={stylePlans.buttonGroup}>
              <button
                ...
                style={{
                  backgroundColor: plansType === "InBound" ? "lightgray" : "",
                }}
              >
                입고 예정 조회
              </button>
              <button
                ...
                style={{
                  backgroundColor: plansType === "OutBound" ? "lightgray" : "",
                }}
              >
                출고 예정 조회
              </button>
                  ...
                  {loading ? (
                    ...
                    currentPosts.map((item, index) => {
                      const date =
                        ...
                        "-";
                      const product =
                        item.GDS_NM || item.GDS_CD || "상품정보없음";
                      ...
                      let statusText =
                        item.STATUS ||
                        (plansType === "InBound" ? "입고 대기" : "출고 대기");
                      if (totalQty > 0) {
                        if (doneQty >= totalQty) {
                          statusText =
                            plansType === "InBound" ? "입고 완료" : "출고 완료";
                        } else if (doneQty > 0) {
                          statusText =
                            plansType === "InBound"
                              ? "입고 진행중"
                      ...
                      return (
                        ...
                        >
                          ...
                          <td>
                            ...
                            <span style={{ color: "#999", margin: "0 2px" }}>
                              /
                            </span>
                          ...
                          <td>
                            <span
                              className={`${stylePlans.statusBadge} ${statusText === "완료" ? stylePlans.statusDone : ""}`}
                            >
                              {statusText}
                            </span>
                          </td>
