...
function PlanRegister({ isOpen, onClose, onRegisterSuccess, currentType }) {
      ...
      const fetchRegisterData = async () => {
        try {
          const response = await fetch(
            `${SERVER_URL}/ttik/plans/register-info`,
            {
              method: "GET",
              credentials: "include",
            },
  ...
  useEffect(() => {
    if (currentType === "OutBound" && formData.itemName) {
      const fetchBoxes = async () => {
        try {
          const response = await fetch(
            `${SERVER_URL}/ttik/plans/available-boxes?gdsCd=${formData.itemName}`,
            {
              credentials: "include",
            },
          );
  ...
  const isBrandSelected =
    currentType === "InBound" ? !!formData.targetName : !!formData.brand;
  ...
  const handleChange = (e) => {
    ...
    setFormData((prev) => {
      ...
      if (currentType === "InBound") {
        const bQty =
          name === "boxQuantity" ? Number(value) : Number(prev.boxQuantity);
        const eQty =
          name === "eaQuantity" ? Number(value) : Number(prev.eaQuantity);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (currentType === "InBound") {
        ...
        // 4. [출고] 선택된 박스 정보 담기
        boxesData = [
          { boxCode: formData.boxCode, eaQuantity: formData.eaQuantity },
        ];
      }
      ...
      const response = await fetch(
        `${SERVER_URL}/ttik/plans/${endpoint}/register`,
        {
          method: "POST",
  ...
  return (
    <>
      <div className={stylePlans.modalOverlay}>
        <div
          className={stylePlans.modalContent}
          onClick={(e) => e.stopPropagation()}
        >
          <button className={stylePlans.closeBtn} onClick={onClose}>
            &times;
          </button>
          <h2>
            {currentType === "InBound"
          ...
          <form onSubmit={handleSubmit}>
                ...
                >
                  ...
                  {currentType === "InBound"
                    ? selectOptions.brands?.map((b) => (
                        <option key={b.PARTNER_SN} value={b.PARTNER_SN}>
                          {b.PARTNER_NM}
                        </option>
                      ))
                    : selectOptions.partners?.map((p) => (
                        <option key={p.PARTNER_SN} value={p.PARTNER_SN}>
                          {p.PARTNER_NM}
                        </option>
                      ))}
                </select>
              ...
              <div className={stylePlans.inputBox}>
                ...
                >
                  ...
                  {selectOptions.categories?.map((c) => (
                    <option key={c.CATE_CD} value={c.CATE_CD}>
                      {c.CATE_NM}
                    </option>
                  ))}
                </select>
              ...
              <div className={stylePlans.inputBoxL}>
                ...
                >
                  ...
                  {selectOptions.Product?.filter((item) => {
                    ...
                    return (
                      (!filterSN ||
                        String(item.BRAND_SN) === String(filterSN)) &&
                      (!formData.category ||
                        String(item.GDS_CAT_CD) === String(formData.category))
                    );
                  }).map((i) => (
                    <option key={i.GDS_CD} value={i.GDS_CD}>
                      {i.GDS_NM}
                    </option>
                  ))}
                </select>
                ...
                <div className={stylePlans.modalItem}>
                  ...
                  >
                    ...
                    {availableBoxes.map((box) => (
                      <option key={box.boxCd} value={box.boxCd}>
                        {box.boxCd}
                      </option>
                    ))}
                  </select>
            ...
            <div
              className={stylePlans.modalItem}
              style={{
                marginTop: "20px",
            ...
            >
              <label>
                ...
                <strong style={{ color: "#e74c3c" }}>
                  {formData.quantity.toLocaleString()} EA
                </strong>
              </label>
            </div>

            <button type="submit" className={stylePlans.submitBtn}>
              등록하기
            </button>
          </form>
        </div>
