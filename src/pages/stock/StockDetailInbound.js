...
function StockDetailInbound() {
  ...
  const selectedBoxCount = scanHistory.filter(
    (h) => checkedItems.has(h.barcode) && h.isBox,
  ).length;
  ...
  const handleBarcodeScanned = async (fullBarcode) => {
    ...
    try {
      ...
      const res = await fetch(
        `${SERVER_URL}/ttik/productdetail/scan-check/${fullBarcode}`,
        {
          method: "GET",
          credentials: "include",
          headers: {
      ...
      if (currentSum + incrementValue > limitQty) {
        setAlert({
          ...
          message: (
            <span>
              ❌ 초과 차단! 더 이상 스캔할 수 없습니다.
              <br />
              (예정: {limitQty} / 현재: {currentSum} / 추가시도:{" "}
              {incrementValue})
            </span>
          ),
          onConfirm: closeAlert,
  ...
  const handleRegister = async () => {
    const itemsToProcess = scanHistory.filter((h) =>
      checkedItems.has(h.barcode),
    );
        ...
        try {
          ...
          for (const item of itemsToProcess) {
            ...
            const response = await fetch(
              `${SERVER_URL}/ttik/productdetail/inbound/process`,
              {
                method: "POST",
                credentials: "include",
          ...
          setAlert({
            ...
            onConfirm: () => {
              ...
              const remainingHistory = scanHistory.filter(
                (h) => !checkedItems.has(h.barcode),
              );
          ...
          setAlert({
            ...
            title: "Error",
            message:
              "처리 중 오류가 발생했습니다. 콘솔창(F12)의 에러 메시지를 확인해주세요.",
            onConfirm: closeAlert,
          });
        }
      },
            ...
            <div className={styles.selectBox}>
              ...
              >
                ...
                {selectOptions.storages?.map((s) => (
                  <option key={s.STR_CD} value={s.STR_CD}>
                    {s.STR_NM}
                  </option>
                ))}
              </select>
              ...
              <select
                ...
                style={{
                  backgroundColor: !selections.storage ? "#f7fafc" : "white",
                }}
              >
                ...
                {selectOptions.zones
                  ?.filter((z) => {
                    ...
                    return (
                      !selections.storage ||
                      String(storageKey) === String(selections.storage)
                    );
                  })
                  .map((z) => (
                    <option
                      key={z.ZONE_SN || z.ZONE_CD}
                      value={z.ZONE_SN || z.ZONE_CD}
                    >
                      {z.ZONE_NM}
                    </option>
                  ))}
              ...
              <select
                ...
                style={{
                  backgroundColor: !selections.zone ? "#f7fafc" : "white",
                }}
              >
                ...
                {selectOptions.racks
                  ?.filter((r) => {
                    ...
                    return (
                      !selections.zone ||
                      String(zoneKey) === String(selections.zone)
                    );
                  })
                  .map((r) => (
                    <option
                      key={r.RACK_SN || r.RACK_CD}
                      value={r.RACK_SN || r.RACK_CD}
                    >
                      {r.RACK_NM}
                    </option>
                  ))}
        ...
        <div className={styles.contentGrid}>
                ...
                >
                  <input
                    ...
                    checked={
                      ...
                      checkedItems.size === scanHistory.length
                    }
                  />
                  <span>전체선택</span>
                </label>
              ...
              <div className={styles.historyList}>
                {scanHistory.length > 0 ? (
                  scanHistory.map((h, i) => (
                    <div
                      key={i}
                      className={`${styles.historyRow} ${h.isBox ? styles.boxLog : ""}`}
                      style={{
                        display: "flex",
                        alignItems: "center",
                        padding: "10px",
          ...
          <div className={styles.leftCol}>
            <div className={styles.card}>
              ...
              >
                <div
                  ...
                  style={{
                    ...
                    backgroundColor:
                      selectedTotalQty === product.stkQty
                        ? "#38a169"
                        : "#3182ce",
              ...
              >
                진행률:{" "}
                {Math.floor((selectedTotalQty / (product.stkQty || 1)) * 100)}%
              </div>
            </div>
            ...
            <button
              ...
              style={{
                ...
                marginTop: "20px",
                backgroundColor:
                  checkedItems.size > 0 && selections.rack
                    ? "#3182ce"
                    : "#a0aec0",
                color: "white",
                border: "none",
