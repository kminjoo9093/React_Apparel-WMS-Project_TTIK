...
function StockDetailOutbound() {
  ...
  const totalCompletedQty =
    (product?.completedOutboundQty || 0) + selectedTotalQty;
  ...
  const handleBarcodeScanned = async (fullBarcode) => {
    ...
    try {
      const response = await fetch(
        `${SERVER_URL}/ttik/productdetail/scan-check/${fullBarcode}?type=OutBound`,
        {
          method: "GET",
          credentials: "include",
          headers: {
      ...
      const currentTotal = scanHistoryRef.current.reduce(
      ...
      onConfirm: async () => {
        try {
          ...
          for (const item of itemsToProcess) {
            ...
            const response = await fetch(
              `${SERVER_URL}/ttik/productdetail/outbound/process`,
              {
                method: "POST",
                credentials: "include",
  ...
  if (!product)
    return <div className={styles.loading}>출고 데이터를 불러오는 중...</div>;
      ...
      <div className={styles.container}>
        <div className={styles.headerSection}>
          ...
          <div className={styles.btnGroup}>
            ...
            <button
              className={`${styles.btn} ${styles.btnScanner}`}
        ...
        {isScannerOpen && (
          <div id="outbound-reader" className={styles.readerBox}></div>
        )}
              ...
              <div className={styles.historyList}>
                {scanHistory.map((h, i) => (
                  <div key={i} className={styles.historyRow}>
                    ...
                    <div style={{ color: "#3182ce", fontWeight: "bold" }}>
                      +{h.increment} EA
                    </div>
                  </div>
                ))}
