...
function StorageList({ storageList: storageOptions = [] }) {
  ...
  const openDetailModal = async (rackSn) => {
    ...
    try {
      const res = await fetch(
        `${SERVER_URL}/ttik/storage/rack/detail?rackSn=${rackSn}`,
        {
          method: "GET",
          credentials: "include",
        },
  ...
  const getStorageListData = async () => {
    ...
    try {
      const res = await fetch(
        `${SERVER_URL}/ttik/storage/listByRack?page=${currentPage - 1}&size=${postsPerPage}&filter=${storageFilter}`,
        {
          method: "GET",
          credentials: "include",
        },
  ...
  return (
    <>
      ...
      <table className={styleStorage.storageTable}>
        ...
        <tbody>
          {isLoading ? (
            <div className={styleStorage.loading}>
              데이터를 불러오는 중입니다
            </div>
          ) : (
      ...
      {isOpen && (
        <div className={styleModal.modalOverlay}>
          <div
            className={`${styleProdModal.modal} ${styleStorage.storageModal}`}
          >
            <div
            ...
            >
              ...
              <div className={styleStorage.modalTableArea}>
                <table className={styleStorage.storageTable}>
                  ...
                  <tbody>
                    {rackDetailList?.boxes &&
                    rackDetailList.boxes.length > 0 ? (
