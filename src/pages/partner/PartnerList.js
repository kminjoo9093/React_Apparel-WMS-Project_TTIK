...
function PartnerList() {
  ...
  const filteredPartners = partners.filter((partner) => {
    const nameMatch = partner.partnerNm
      ?.toLowerCase()
      ...
      onConfirm: async () => {
        try {
          ...
          if (response.ok) {
            ...
            // 로컬 상태 즉시 반영
            setPartners((prev) =>
              prev.filter((p) => !selectedIds.includes(p.partnerSn)),
            );
            setSelectedIds([]);
          } else {
            setAlert({
      ...
      <div>
        ...
        <div className={stylePartner.partnerListbox}>
          <div className={stylePartner.searchBox}>
            ...
            />
            <button
              className={stylePartner.partnerBtn}
              onClick={() => setIsModalOpen(true)}
            >
              거래처 등록
            </button>
            <button className={stylePartner.partnerBtn} onClick={handleDelete}>
              삭제
            </button>
          </div>
              ...
              <table className={stylePartner.partnerTable}>
                ...
                <tbody>
                  {currentPartners.length > 0 ? (
                      ...
                      <tr key={partner.partnerSn}>
                        <td>
                          <input
                            ...
                            onChange={() =>
                              handleCheckboxChange(partner.partnerSn)
                            }
                          />
                        </td>
                    ...
                    <tr>
                      <td colSpan="4" className={stylePartner.noData}>
                        데이터가 없습니다.
                      </td>
                    </tr>
                  )}
