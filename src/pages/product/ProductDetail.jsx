...
const ProductDetail = () => {
  ...
  const fetchProductData = async () => {
    try {
      ...
      const response = await axios.get(
        `${SERVER_URL}/ttik/product/productDetail/${gds_cd}`,
        {
  ...
  const handleDelete = async (product) => {
    ...
    const currentStock = Number(
      product.STK_QTY ?? product.stk_qty ?? product.frst_stk_qty ?? 0,
    );
  ...
  return (
    <>
      <Alert {...alert} />
      <div
        className={style["card-stack-wrapper"]}
        ...
        <AnimatePresence mode="popLayout">
          {cardList.slice(0, 3).map((product, index) => {
            ...
            const threshold = Number(
              product.THRESHOLD || product.threshold || 0,
            );
              ...
              >
                ...
                >
                  ...
                  >
                    <div>
                      ...
                      >
                        {product.SEASON_NM || product.season_nm} | {gdsCd}
                      </p>
                    </div>
                    {isLowStock ? (
                      <div
                ...
                >
                  ...
                  <div className={style["info-item"]}>
                    <span className={style["info-label"]}>시즌</span>
                    <span className={style["info-value"]}>
                      {product.SEASON_NM || product.season_nm || "-"}
                    </span>
                  </div>
                  <div className={style["info-item"]}>
                    <span className={style["info-label"]}>브랜드</span>
                    <span
                      className={style["info-value"]}
                      style={{ color: "#2a9426", fontWeight: "bold" }}
                    >
                      {product.BRAND_NM || product.brand_nm || "-"}
                    </span>
                  </div>
                  <div className={style["info-item"]}>
                    <span className={style["info-label"]}>상품 코드</span>
                    <span className={style["info-value"]}>
                      {product.GDS_CD || product.gds_cd}
                    </span>
                  </div>
                  <div className={style["info-item"]}>
                    <span className={style["info-label"]}>상품 명</span>
                    <span
                      className={style["info-value"]}
                      style={{ color: "#003cff", fontWeight: "bold" }}
                    >
                      {product.GDS_NM || product.gds_nm}
                    </span>
                  </div>
                  <div className={style["info-item"]}>
                    <span className={style["info-label"]}>단가</span>
                    <span className={style["info-value"]}>
                      {untprc.toLocaleString()} 원
                    </span>
                  </div>
                  <div
                    style={{
                ...
                >
                  ...
                  <div className={style["info-item"]}>
                    <span className={style["info-label"]}>카테고리</span>
                    <span className={style["info-value"]}>
                      {product.GDS_CAT_NM || product.gds_cat_nm || "-"}
                    </span>
                  </div>
                  <div className={style["info-item"]}>
                    <span className={style["info-label"]}>사이즈</span>
                    <span className={style["info-value"]}>
                      {product.SIZE_NM || product.size_nm || "-"}
                    </span>
                  </div>
                  <div className={style["info-item"]}>
                    <span className={style["info-label"]}>최초 등록일</span>
                    <span className={style["info-value"]}>
                      {product.FRST_REG_DT || product.frst_reg_dt}
                    </span>
                  </div>
                  <div className={style["info-item"]}>
                    <span className={style["info-label"]}>박스 입수 수량</span>
                    <span className={style["info-value"]}>{inbox} EA</span>
                  </div>
                  <div className={style["info-item"]}>
                    <span className={style["info-label"]}>임계치</span>
                    <span
                      className={style["info-value"]}
                    ...
                    >
                      ...
                      <h4
                        style={{
                          fontSize: "1.5rem",
                          fontWeight: "bold",
                          color: isLowStock ? "#ef4444" : "#3b82f6",
                        }}
                      >
                        {Math.round((stock / (threshold || 1)) * 100)}%
                      </h4>
                    </div>
                  </div>
                </div>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      ...
                      >
                        <XAxis
                          type="number"
                          hide
                          domain={[0, Math.max(stock, threshold) * 1.2]}
                        />
                        <YAxis
                          dataKey="name"
                          type="category"
                ...
                <div
                  style={{ display: "flex", gap: "15px", marginBottom: "50px" }}
                >
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
