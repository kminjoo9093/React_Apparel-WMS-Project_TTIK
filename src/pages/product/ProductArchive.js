    ...
    const fetchArchiveList = async () => {
      try {
        ...
        const res = await axios.get(
          `${SERVER_URL}/ttik/product/productArchive`,
          {
  ...
  return (
    <>
      <div className={style["archive-wrapper"]}>
        <h2 className={style["archive-title"]}>📦 관리 제외 품목 (Archive)</h2>
        ...
        <div className={style["table-container"]}>
          <table className={style["archive-table"]}>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan="5" className={style["status-msg"]}>
                    데이터 불러오는 중...
                  </td>
                </tr>
              ) : errorMsg ? (
